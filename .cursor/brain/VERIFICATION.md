...
Runs: `npm audit --audit-level=moderate` → `typecheck` → `lint:oxlint` → `lint` → `format:check` → `test:coverage` → `build` → `node scripts/check-web-vitals-chunks.mjs` (default chunk check only).
